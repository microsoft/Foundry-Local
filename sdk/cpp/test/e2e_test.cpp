...
// ===========================================================================
// DiscoverEps / DownloadAndRegisterEps
// ===========================================================================

TEST_F(EndToEndTest, DISABLED_DiscoverEps_ReturnsValidResults) {
    if (IsRunningInCI()) {
        GTEST_SKIP() << "Skipped in CI (may require network)";
    }

    std::vector<EpInfo> eps;
    EXPECT_NO_THROW(eps = Manager::Instance().DiscoverEps());

    // Each EP should have a non-empty name
    for (const auto& ep : eps) {
        EXPECT_FALSE(ep.name.empty()) << "EP name should not be empty";
    }
}

TEST_F(EndToEndTest, DISABLED_DownloadAndRegisterEps_Succeeds) {
    if (IsRunningInCI()) {
        GTEST_SKIP() << "Skipped in CI (may require network)";
    }

    EpDownloadResult result;
    EXPECT_NO_THROW(result = Manager::Instance().DownloadAndRegisterEps());
    EXPECT_TRUE(result.success) << "EP download should succeed; status: " << result.status;
}

TEST_F(EndToEndTest, DISABLED_DownloadAndRegisterEps_WithProgress_ReportsProgress) {
    if (IsRunningInCI()) {
        GTEST_SKIP() << "Skipped in CI (may require network)";
    }
    ...
    EpDownloadResult result;
    EXPECT_NO_THROW(result = Manager::Instance().DownloadAndRegisterEps(
        [&](const std::string& epName, double percent) {
            EXPECT_FALSE(epName.empty()) << "Progress EP name should not be empty";
            EXPECT_GE(percent, 0.0) << "Progress should be >= 0";
            EXPECT_LE(percent, 100.0) << "Progress should be <= 100";
...
TEST_F(EndToEndTest, DISABLED_DownloadAndRegisterEps_ByName_Succeeds) {
    ...
    // Discover first, then try downloading the first available EP by name
    auto eps = Manager::Instance().DiscoverEps();
    if (eps.empty()) {
        GTEST_SKIP() << "No EPs available on this device";
    }

    EpDownloadResult result;
    EXPECT_NO_THROW(result = Manager::Instance().DownloadAndRegisterEps({eps[0].name}));
    EXPECT_TRUE(result.success) << "EP download should succeed; status: " << result.status;
}
