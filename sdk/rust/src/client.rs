...
impl HttpClient {
    ...
    pub fn new(host: &str, timeout_secs: Option<u64>) -> Self {
        ...
        let mut client_builder = Client::builder()
            .user_agent("foundry-local-rust-sdk/0.1.0");
    ...
    pub async fn get<T: DeserializeOwned>(&self, path: &str, query_params: Option<&[(&str, &str)]>) -> Result<Option<T>, ClientError> {
    ...
    ///
    /// JSON response.
    pub async fn post<T: DeserializeOwned>(&self, path: &str, body: Option<Value>) -> Result<T, ClientError> {
        let url = format!("{}{}", self.base_url, path);
        debug!("POST {}", url);
    ...
    async fn handle_response<T: DeserializeOwned>(&self, response: Response) -> Result<Option<T>, ClientError> {
        let status = response.status();
        if !status.is_success() {
            let text = response.text().await.unwrap_or_else(|_| "No response text".to_string());
