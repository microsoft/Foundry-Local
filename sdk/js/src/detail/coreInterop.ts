...
const CallbackType = koffi.proto(
  "void CallbackType(void *data, int32_t length, void *userData)",
);
...
export class CoreInterop {
  ...
  private static _resolveDefaultCorePath(config: Configuration): string | null {
    ...
    if (fs.existsSync(corePath)) {
      ...
      if (!("Bootstrap" in config.params)) {
        const bootstrapDllPath = path.join(
          packageDir,
  ...
  constructor(config: Configuration) {
    const corePath =
      config.params["FoundryLocalCorePath"] ||
      CoreInterop._resolveDefaultCorePath(config);

    if (!corePath) {
      throw new Error(
        "FoundryLocalCorePath not specified in configuration and could not auto-discover binaries. Please run 'npm install' to download native libraries.",
      );
    }
    ...
    this.execute_command = this.lib.func(
      "void execute_command(RequestBuffer *request, _Inout_ ResponseBuffer *response)",
    );
    this.execute_command_with_callback = this.lib.func(
      "void execute_command_with_callback(RequestBuffer *request, _Inout_ ResponseBuffer *response, CallbackType *callback, void *userData)",
    );
    this.execute_command_with_binary = this.lib.func(
      "void execute_command_with_binary(StreamingRequestBuffer *request, _Inout_ ResponseBuffer *response)",
    );
  }
   ...
   */
  public executeCommandWithBinary(
    command: string,
    params: any,
  ...
  public executeCommandStreaming(
    command: string,
    params: any,
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      this.execute_command_with_callback.async(
        req,
        res,
        cb,
        null,
