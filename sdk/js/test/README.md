...
1.  **Prepared the Core Libraries**: Run the `download_core_nuget.ps1` script located in `sdk/nuget/`. This script downloads the necessary NuGet packages and extracts the native libraries (dll/so/dylib) into the `sdk/nuget/packages/core/` directory, which the tests are configured to use.
    ```powershell
