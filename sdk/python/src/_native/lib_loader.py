...
3. Development build: walk up from ``__file__`` looking for ``sdk/cpp/build/<Platform>/<Config>/...``
...
def _build_platform_dir() -> str:
    """Return the platform sub-directory under ``sdk/cpp/build/`` (matches the C++ build script)."""
    if sys.platform == "win32":
        return "Windows"
    if sys.platform == "darwin":
        return "macOS"
...
def _dev_build_candidates(sdk_root: pathlib.Path, name: str) -> list[pathlib.Path]:
    """Enumerate development-build paths to probe under ``sdk/cpp/build/<Platform>/<Config>/`` for *name*.
    ...
    base = sdk_root / "cpp" / "build" / platform_dir
...
def find_library() -> pathlib.Path | None:
    ...
    # 3. Development build — walk up the directory tree looking for sdk/ then probe explicit per-platform
    ...
    for parent in pathlib.Path(__file__).resolve().parents:
        sdk_root = parent / "sdk"
        if sdk_root.is_dir():
            for dev_candidate in _dev_build_candidates(sdk_root, name):
                if dev_candidate.exists():
                    return dev_candidate
            break  # Found sdk/ but no library — fall through to system path.
