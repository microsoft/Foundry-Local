...
#   sdk/python/src/_native/build_cffi.py
# Walk up to the repo root (Foundry-Local/) then into sdk/cpp/include/.
_HERE = pathlib.Path(__file__).resolve()
_SDK_PYTHON_DIR = _HERE.parent.parent.parent  # sdk/python/
_REPO_ROOT = _SDK_PYTHON_DIR.parent.parent  # Foundry-Local/
_SDK_DIR = _REPO_ROOT / "sdk"
_INCLUDE_DIR = _SDK_DIR / "cpp" / "include"

if not (_INCLUDE_DIR / "foundry_local" / "foundry_local_c.h").exists():
    raise FileNotFoundError(
        f"Could not locate foundry_local_c.h under {_INCLUDE_DIR}. "
        "Run this script from the sdk/python/ directory inside the repo."
    )
...
#   DLL:  sdk/cpp/build/Windows/RelWithDebInfo/bin/RelWithDebInfo/
#   .lib: sdk/cpp/build/Windows/RelWithDebInfo/RelWithDebInfo/
...
if _sys.platform == "win32":
    ...
    #      (sdk/python/src/foundry_local_sdk/_native/<rid>/foundry_local.lib).
    ...
    #      (sdk/cpp/build/Windows/<Config>/<Config>/foundry_local.lib).
    ...
    _lib_candidates = [
        ...
        _SDK_DIR / "cpp" / "build" / "Windows" / "RelWithDebInfo" / "RelWithDebInfo" / "foundry_local.lib",
        _SDK_DIR / "cpp" / "build" / "Windows" / "Debug" / "Debug" / "foundry_local.lib",
    ]
    ...
    # sdk/cpp/build/macOS/<Config>/.
    _lib_candidates = [
        _HERE.parent / "osx-arm64" / "libfoundry_local.dylib",
        _HERE.parent / "osx-x64" / "libfoundry_local.dylib",
        _SDK_DIR / "cpp" / "build" / "macOS" / "RelWithDebInfo" / "libfoundry_local.dylib",
        _SDK_DIR / "cpp" / "build" / "macOS" / "Debug" / "libfoundry_local.dylib",
    ]
    for _lib_candidate in _lib_candidates:
        if _lib_candidate.exists():
            _dev_library_dirs = [str(_lib_candidate.parent)]
    ...
    _lib_candidates = [
        ...
        _SDK_DIR / "cpp" / "build" / "Linux" / "RelWithDebInfo" / "libfoundry_local.so",
        _SDK_DIR / "cpp" / "build" / "Linux" / "Debug" / "libfoundry_local.so",
    ]
    for _lib_candidate in _lib_candidates:
        if _lib_candidate.exists():
            _dev_library_dirs = [str(_lib_candidate.parent)]
...
if __name__ == "__main__":
    ...
    # In the legacy layout, src/ IS the foundry_local_sdk package.
    _src_dir = str(_HERE.parent.parent)  # sdk/python/src/
