...
Pass a `threading.Event` as `cancel_event` to the cancellable download APIs. Set the event from another thread or handler to cancel the in-progress download.
...
manager.download_and_register_eps_cancellable(cancel_event=cancel_event)
model.download_cancellable(cancel_event=cancel_event)
```
