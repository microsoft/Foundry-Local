...
The WinML variant always installs `foundry-local-core-winml`, `onnxruntime-core`, `onnxruntime-genai-core`. The standard and WinML variants are mutually exclusive — install only one per environment. WinML is auto-detected at runtime: if the WinML package is installed, the SDK automatically enables the Windows App Runtime Bootstrap.
