...
def test_public_symbols_importable():
    mod = importlib.import_module("foundry_local")
...
def test_model_is_alias_for_imodel():
    from foundry_local import IModel, Model
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "foundry_local.catalog",
        "foundry_local.imodel",
        "foundry_local.items",
        "foundry_local.request",
        "foundry_local.response",
        "foundry_local.session",
        "foundry_local.session_types",
        "foundry_local.foundry_local_manager",
    ],
)
...
class TestOpenAiImportGuard:
    ...
    def test_individual_clients_do_not_have_their_own_guard(self):
        ...
        pkg_dir = Path(__file__).resolve().parents[2] / "src" / "foundry_local" / "openai"
        for filename in ("chat_client.py", "embedding_client.py", "audio_client.py"):
            ...
            assert "except ImportError" not in src, (
                ...
                f"the package-level guard in foundry_local.openai.__init__ "
