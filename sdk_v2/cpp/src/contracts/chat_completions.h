...
struct ChatCompletionMessage {
  ...
  // for role="assistant": the calls this message issued. Parsed into the same typed shape the
  // response side emits, so a transcript round-trips through one representation rather than two,
  // and a custom call keeps its raw text instead of being read as JSON arguments.
...
///
/// A custom tool takes a single raw text payload rather than a JSON argument object. Only the text
/// format is supported, so nothing about the format needs to be carried past parsing — a format
/// this runtime cannot honour is rejected when the request is read.
...
struct ChatCompletionToolChoice {
  enum class Kind { kAuto, kNone, kRequired, kFunction, kCustom };
