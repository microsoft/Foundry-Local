...
void from_json(const nlohmann::json& j, ChatCompletionCustomCall& c) {
  ...
  // A custom payload is raw text and stays raw text. Coercing a non-string into one is exactly what must not happen
  // to a payload the tool reads verbatim, so anything else is rejected rather than stringified.
  if (auto input = j.find("input"); input != j.end() && !input->is_null()) {
    if (!input->is_string()) {
      FL_THROW(FOUNDRY_LOCAL_ERROR_INVALID_ARGUMENT, "tool_calls[].custom.input must be a string");
    }
...
void from_json(const nlohmann::json& j, ChatCompletionCustomToolDef& c) {
  ...
  // Validated but not stored: text is the only supported shape, so a format that survives
  // validation carries no information the rest of the pipeline needs.
  auto format = j.find("format");
  tools::ValidateCustomToolFormat(format == j.end() ? nlohmann::json() : *format, c.name);
}
...
void to_json(nlohmann::json& j, const ChatCompletionCustomToolDef& c) {
  ...
  // Always spelled out: text is the only format the runtime accepts, so echoing it makes the
  // declaration this tool actually runs under explicit rather than implied by omission.
  j["format"] = nlohmann::json{{"type", "text"}};
}
