...
///
/// Only the text format is supported, so nothing about the format survives parsing — a format this
/// runtime cannot honour is rejected when the request is read.
...
using ToolChoice = std::variant<std::string, ForcedFunction, ForcedCustomTool>;
