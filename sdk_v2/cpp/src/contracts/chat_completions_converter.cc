...
ToolKind KindOf(const ToolKindsByName& kinds, const std::string& name) {
  ...
  return kind == kinds.end() ? ToolKind::kFunction : kind->second;
}
...
void BuildRequestItems(const ChatCompletionRequest& req, Request& session_request) {
  for (const auto& msg : req.messages) {
    ...
    for (const auto& call : msg.tool_calls) {
      ...
      session_request.AddOwnedItem(std::make_unique<ToolCallItem>(call.id, call.Name(), call.Payload(), kind));
    }
  }
}
  ...
  for (const auto& item : response.items) {
    if (item->type == FOUNDRY_LOCAL_ITEM_MESSAGE) {
      ...
      tool_calls.push_back(
          MakeToolCall(tc_item.call_id, tc_item.name, tc_item.arguments, KindOf(tool_kinds, tc_item.name)));
    }
  }
