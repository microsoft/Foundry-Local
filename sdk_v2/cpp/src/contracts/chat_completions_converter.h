...
using ToolKindsByName = std::unordered_map<std::string, ToolKind>;
...
ToolKind KindOf(const ToolKindsByName& kinds, const std::string& name);
...
///
/// @param tool_kinds  Kinds this turn was prompted with. Empty means every call is a function call,
///                    which is the shape a request that declared no custom tools can produce.
