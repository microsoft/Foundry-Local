...
class BaseModelCatalog : public ICatalog {
 ...
 protected:
  ...
  virtual std::vector<Model> FetchModelVersions(const std::string& model_alias) const = 0;
  ...
  virtual std::vector<Model> FetchModelsByIds(const std::vector<std::string>& model_ids) const = 0;
