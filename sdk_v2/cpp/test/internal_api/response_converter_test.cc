...
TEST(ResponseConverterTest, BuildToolCallStreamOutput_FunctionCall_EmitsCompleteLifecycle) {
  ...
  auto output = BuildToolCallStreamOutput(call, fl::ToolKind::kFunction, 3, sequence_number);
...
TEST(ResponseConverterTest, ExtractResponsesToolDefinitions_AllowedToolsAndForcedFunction_BothApplied) {
  ...
  // Result: no tools (strict intersection — matches the reference behaviour). tool_choice still
  // gets "required" since the forced-tool branch sets it before allowed_tools runs; no tools
  // combined with "required" effectively disables tool calling, which is the intended consequence
  // of an over-restricted allowed_tools list.
  auto params = MakeToolParams();
  params.tools = std::vector<responses::ToolDefinition>{
      MakeTool("tool_a"), MakeTool("tool_b"), MakeTool("tool_c")};
