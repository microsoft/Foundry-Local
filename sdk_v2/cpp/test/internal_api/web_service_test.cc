...
TEST_F(WebServiceTest, ChatCompletionsRejectsCustomToolWithGrammarFormat) {
  const json grammar_format = {{"type", "grammar"},
  ...
  json body = {
      ...
      {"tools", json::array({{{"type", "custom"},
                              {"custom", {{"name", "apply_patch"}, {"format", grammar_format}}}}})},
  };
...
TEST_F(WebServiceTest, ChatCompletionsAcceptsMixedFunctionAndCustomToolInventory) {
  json body = {
...
TEST_F(WebServiceTest, ResponsesRejectsCustomToolWithGrammarFormat) {
  json body = {
      {"model", test::kLoadableTestModelAlias},
      {"input", "hi"},
      {"tools", json::array({{{"type", "custom"},
