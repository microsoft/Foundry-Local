...
ResponseConverter::ToolKindsByName KindsOf(const std::vector<fl::ToolDefinition>& definitions) {
  ResponseConverter::ToolKindsByName kinds;
...
TEST(ResponsesCustomToolDeclarationTest, RejectsGrammarAndUnknownFormats) {
...
TEST(ResponsesAllowedToolsTest, ExcludingAForcedCustomToolLeavesNoTools) {
  auto params = ParamsWithTools(R"([
    {"type": "custom", "name": "apply_patch"},
    {"type": "function", "name": "bash", "parameters": {"type": "object"}}
  ])");
  ...
  Request session_request;
  EXPECT_TRUE(ResponseConverter::ExtractResponsesToolDefinitions(params, session_request).empty());
  EXPECT_EQ(std::string(session_request.options.Find("tool_choice")), "required");
}
...
TEST(ResponsesToolCallEmissionTest, CustomCallBecomesACustomToolCallItem) {
  auto params = ParamsWithTools(R"([{"type": "custom", "name": "apply_patch"}])");
  Request session_request;
  const auto kinds = KindsOf(ResponseConverter::ExtractResponsesToolDefinitions(params, session_request));

  std::vector<std::unique_ptr<Item>> items;
  items.push_back(std::make_unique<ToolCallItem>("call_2", "apply_patch", kPatch));

  auto [output, output_text] = ResponseConverter::FromSessionResponse(ResponseWithCalls(std::move(items)), kinds);
  ...
  EXPECT_EQ(custom.status, ResponseStatus::kCompleted);
...
TEST(ResponsesToolCallEmissionTest, SimultaneousCallsDoNotCrossAssociate) {
  ResponseConverter::ToolKindsByName kinds{{"bash", fl::ToolKind::kFunction},
                                           {"apply_patch", fl::ToolKind::kCustom}};

  std::vector<std::unique_ptr<Item>> items;
  items.push_back(std::make_unique<ToolCallItem>("call_fn", "bash", R"({"command":"ls"})"));
  items.push_back(std::make_unique<ToolCallItem>("call_custom", "apply_patch", kPatch));
...
TEST(ResponsesToolCallEmissionTest, UndeclaredNameIsReportedAsAFunctionCall) {
  ...
  auto [output, output_text] = ResponseConverter::FromSessionResponse(ResponseWithCalls(std::move(items)), {});
...
TEST(ResponsesToolCallEmissionTest, CustomCallStreamsTheCompleteInputLifecycle) {
  ToolCallItem call("call_2", "apply_patch", kPatch);
  int sequence_number = 5;

  auto output = ResponseConverter::BuildToolCallStreamOutput(call, fl::ToolKind::kCustom, 2, sequence_number);
  ...
  EXPECT_EQ(delta.at("item_id"), completed.id);
  EXPECT_EQ(delta.at("call_id"), "call_2");
  EXPECT_EQ(delta.at("output_index"), 2);

  const json done = output.events[2];
  EXPECT_EQ(done.at("type"), "response.custom_tool_call_input.done");
  EXPECT_EQ(done.at("input"), kPatch);
  EXPECT_EQ(done.at("name"), "apply_patch");
  EXPECT_EQ(done.at("call_id"), "call_2");
  EXPECT_FALSE(done.contains("arguments")) << "a custom payload is never reported as arguments";

  const json item_done = output.events[3];
  EXPECT_EQ(item_done.at("type"), "response.output_item.done");
  EXPECT_EQ(item_done.at("item").at("type"), "custom_tool_call");
  EXPECT_EQ(item_done.at("item").at("input"), kPatch);
  EXPECT_EQ(item_done.at("item").at("status"), "completed");
}

TEST(ResponsesToolCallEmissionTest, StreamedAndFinalItemsAgreeOnCallIdAndInput) {
  ResponseConverter::ToolKindsByName kinds{{"apply_patch", fl::ToolKind::kCustom}};
  ToolCallItem call("call_2", "apply_patch", kPatch);

  int sequence_number = 0;
  auto streamed = ResponseConverter::BuildToolCallStreamOutput(call, fl::ToolKind::kCustom, 0, sequence_number);
...
TEST(ResponsesToolCallEmissionTest, FunctionCallLifecycleIsUnchanged) {
  ...
  auto output = ResponseConverter::BuildToolCallStreamOutput(call, fl::ToolKind::kFunction, 0, sequence_number);
...
TEST(ResponsesToolCallEmissionTest, CompletedResponseCarriesTheCustomCallForTheNextTurn) {
  ...
  EXPECT_EQ(replayed_call.declared_kind, ToolKind::kCustom) << "the stored call states its own kind on replay";
