...
ToolKindsByName KindsOf(const std::vector<ToolDefinition>& definitions) {
  ToolKindsByName kinds;
...
TEST(ChatCustomToolDeclarationTest, RejectsGrammarFormat) {
...
TEST(ChatToolCallEmissionTest, UndeclaredNameIsReportedAsAFunctionCall) {
  // Without a declaration there is no typed context to read the call with, so it is reported
  // exactly as the model produced it rather than guessed at.
  ToolKindsByName kinds{{"apply_patch", ToolKind::kCustom}};
  EXPECT_EQ(KindOf(kinds, "unknown_tool"), ToolKind::kFunction);
  EXPECT_EQ(KindOf(kinds, "apply_patch"), ToolKind::kCustom);
  EXPECT_EQ(KindOf({}, "apply_patch"), ToolKind::kFunction);
}
...
TEST(ChatToolCallEmissionTest, BuildResponseReportsEachCallUnderItsDeclaredKind) {
  ...
  Request session_request;
  const auto kinds = KindsOf(ExtractToolDefinitions(req, session_request));

  std::vector<std::unique_ptr<Item>> items;
  items.push_back(std::make_unique<ToolCallItem>("call_1", "bash", R"({"command":"ls"})"));
  items.push_back(std::make_unique<ToolCallItem>("call_2", "apply_patch", kPatch));
  auto response = ResponseWithCalls(std::move(items));

  auto built = BuildResponse(response, "chatcmpl-1", 100, "m", kinds);
...
TEST(ChatToolCallEmissionTest, SimultaneousCallsDoNotCrossAssociate) {
  ToolKindsByName kinds{{"bash", ToolKind::kFunction}, {"apply_patch", ToolKind::kCustom}};
  ...
  auto calls = ToolCallsOf(BuildResponse(response, "chatcmpl-1", 100, "m", kinds));
...
TEST(ChatToolCallEmissionTest, StreamedChunkAndFinalMessageAgreeOnIdAndPayload) {
  ...
  auto streamed = MakeToolCall("call_abc", "apply_patch", kPatch, KindOf(kinds, "apply_patch"));
  streamed.index = 0;
  ...
  std::vector<std::unique_ptr<Item>> items;
  items.push_back(std::make_unique<ToolCallItem>("call_abc", "apply_patch", kPatch));
  auto final_call = ToolCallsOf(BuildResponse(ResponseWithCalls(std::move(items)), "chatcmpl-1", 100, "m", kinds))
                        .at(0);
...
TEST(ChatToolCallEmissionTest, RawInputWithJsonLikeContentIsNotCoerced) {
  ...
  std::vector<std::unique_ptr<Item>> items;
  items.push_back(std::make_unique<ToolCallItem>("call_1", "apply_patch", payload));

  auto calls = ToolCallsOf(BuildResponse(ResponseWithCalls(std::move(items)), "chatcmpl-1", 100, "m", kinds));
  EXPECT_EQ(calls.at(0).at("custom").at("input"), payload);
}

TEST(ChatToolCallEmissionTest, CancelledTurnReportsNoCompletedCall) {
  ToolKindsByName kinds{{"apply_patch", ToolKind::kCustom}};

  Response response;
  response.items.push_back(std::make_unique<ToolCallItem>("call_1", "apply_patch", kPatch));
  response.finish_reason = FOUNDRY_LOCAL_FINISH_NONE;

  const json built = BuildResponse(response, "chatcmpl-1", 100, "m", kinds);
  EXPECT_FALSE(built.at("choices").at(0).at("message").contains("tool_calls"));
}
