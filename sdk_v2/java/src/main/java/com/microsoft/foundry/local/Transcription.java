...
public final class Transcription implements AutoCloseable {
    ...
    Transcription(AudioSession session, byte[] wav, PcmFormat format, Consumer<SpeechEvent> listener) {
        ...
        request = api.create(api.inference, 0);
        try {
            Pointer audio = api.create(api.item, 0, 30);
            ...
            try (Memory text = NativeApi.utf8("pcm")) {
                ...
                data.write();
                api.check(api.item.pointer(8, audio, data));
                api.check(api.inference.pointer(2, request, audio, (byte) 1));
                transferred = true;
            } finally {
                if (!transferred) api.item.call(1, audio);
            }
            {
                Pointer queueItem = api.create(api.item, 0, 200);
                boolean queueTransferred = false;
                try {
                    queue = api.create(api.item, 23, queueItem);
                    api.check(api.inference.pointer(2, request, queueItem, (byte) 1));
                    queueTransferred = true;
                } finally {
                    if (!queueTransferred) api.item.call(1, queueItem);
                }
            }
            api.check(api.inference.pointer(15, session.handle, callback, null));
        } catch (RuntimeException | Error e) {
            api.inference.call(1, request);
            request = null;
            throw e;
        }
    ...
    public synchronized void writePcm(byte[] bytes) throws InterruptedException {
        ...
        try {
            ...
            data.write();
            api.check(api.item.pointer(3, item, data));
            ownsBuffer = true;
            api.check(api.item.pointer(26, queue, item));
            if (firstInputNanos == 0) firstInputNanos = System.nanoTime();
            submittedBytes += bytes.length;
            item = null; // The native queue now owns the item and its deleter.
        } finally {
            if (item != null) api.item.call(1, item);
    ...
    private int onEvent(NativeApi.CallbackData event, Pointer userData) {
        ...
        try {
            ...
            while (api.item.bool(27, event.queue, next)) {
                Pointer item = next.getValue();
                try {
                    if (api.item.integer(2, item) != 31) throw new IllegalStateException("Unexpected ASR stream item");
                    NativeApi.SegmentData data = new NativeApi.SegmentData();
                    data.write();
                    api.check(api.item.pointer(19, item, data));
                    data.read();
    ...
    private void run() {
        ...
        try {
            Pointer status = api.inference.pointer(17, session.handle, request, response);
            if (callbackFailure.get() != null) {
                if (status != null) api.root.call(1, status);
    ...
    @Override public void close() {
        ...
        try {
            try {
                api.check(api.inference.pointer(15, session.handle, null, null));
            } finally {
                api.inference.call(1, request);
                synchronized (this) {
                    request = null;
