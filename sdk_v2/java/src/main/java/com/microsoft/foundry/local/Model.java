...
public final class Model {
    ...
    public ModelInfo info() {
        ...
        synchronized (owner) {
            ...
            Pointer info = api.create(api.model, 0, handle);
            return new ModelInfo(NativeApi.text(api.model.pointer(11, info)),
                    NativeApi.text(api.model.pointer(14, info)), NativeApi.text(api.model.pointer(12, info)),
                    api.model.integer(13, info), NativeApi.text(api.model.pointer(15, info)),
                    NativeApi.text(api.model.pointer(17, info)), NativeApi.text(api.model.pointer(18, info)),
                    NativeApi.text(api.model.pointer(21, info, "license")),
                    NativeApi.text(api.model.pointer(21, info, "license_description")));
        }
    }

    public boolean isCached() { return flag(2); }
    public boolean isLoaded() { return flag(5); }
    ...
    public Path path() {
        ...
        synchronized (owner) {
            ...
            return Path.of(NativeApi.text(owner.api.create(owner.api.model, 3, handle)));
        }
    }
    ...
    public void download(CancellationToken cancellation, DoubleConsumer progress) {
        ...
        synchronized (owner) {
            ...
            Pointer status;
            try { status = owner.api.model.pointer(4, handle, callback, null); }
            finally { Reference.reachabilityFence(callback); }
            if (failure.get() != null) {
                if (status != null) owner.api.root.call(1, status);
    ...
    public void load() {
        ...
        synchronized (owner) {
            ...
            owner.api.check(owner.api.model.pointer(6, handle));
        }
    }
    ...
    public void unload() {
        ...
        synchronized (owner) {
            ...
            owner.api.check(owner.api.model.pointer(7, handle));
        }
    }
